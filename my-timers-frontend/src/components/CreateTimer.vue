  ...
  methods: {
    createTimer() {
      ...
      if (date < Math.floor(Date.now() / 1000)) {
        ...
        // Replaced id generated by backend by following line on request to deatch forntend from backend
        let id = Math.floor(Math.random() * 1000000000);

        if (!this.name) this.name = i18n.t("noTitle");
        ...
        // All timers data moved from Vuex store to localStorage as requested
        ...
        if (
          ...
          !(JSON.parse(localStorage.getItem("timers")) instanceof Array)
        ) {
          localStorage.setItem(
            "timers",
            JSON.stringify([{ date, title: this.name, id }])
          );
